...
Using `pivot_longer` and `pivot_wider` you can compare variables in a table side by side:
...
`#extension` If you don't like boxplots you can use a point-range plot. Bar charts are not recommended because readers perceive points inside the bar, e.g. bottom error bar, as more likely than those outside, e.g. top error bar, even though this is not the case (TODO add citation).
...
`#extension` This is useful for calculating effect size measures but less useful for inference:
